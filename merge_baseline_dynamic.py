#!/usr/bin/env python3
"""
合併 Baseline Dynamic 場景的暫存文件

用法: python3 merge_baseline_dynamic.py p1
     python3 merge_baseline_dynamic.py p5
     python3 merge_baseline_dynamic.py p10
"""
...
if len(sys.argv) != 2:
    print("用法: python3 merge_baseline_dynamic.py <scenario>")
    print("  scenario: p1, p5, p10")
    sys.exit(1)

scenario = sys.argv[1]
temp_dir = f"paper/satellite_networks_state/analytic_result/temp_baseline_dynamic_{scenario}"
output_file = f"paper/satellite_networks_state/analytic_result/baseline_dynamic_{scenario}_signaling_stats.json"

print(f"合併 Baseline Dynamic {scenario.upper()} 暫存文件")
