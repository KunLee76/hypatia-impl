...
# 優化版本：使用 Dijkstra 替代 Floyd-Warshall
# ============================================
...
from .fstate_calculation import calculate_fstate_dijkstra_based
import os
import io
import sys
import tempfile
...
class VirtualPIDRouter:
    def __init__(self, grid_deg=GRID_DEG, lon_min=-180, lon_max=180, lat_min=-90, lat_max=90,
        ...
        self.num_pid = self.num_lon * self.num_lat
        ...
        # 靜態：PID 鄰接（4 或 8 鄰）
        self.pid_neighbors: Dict[int, Set[int]] = self._build_pid_neighbors()
        _alog("[PID] router init", {
    ...
    def pid_of(self, lat: float, lon: float) -> int:
        lon_adj = lon
    ...
    def _build_pid_neighbors(self) -> Dict[int, Set[int]]:
        neigh = {pid: set() for pid in range(self.num_pid)}
        dirs = [(-1,0),(1,0),(0,-1),(0,1)]
        if self.allow_diag:
            dirs += [(-1,-1),(-1,1),(1,-1),(1,1)]
        for lat_idx in range(self.num_lat):
            for lon_idx in range(self.num_lon):
                pid = lat_idx * self.num_lon + lon_idx
                for dx, dy in dirs:
    ...
    def _pid_center(self, pid: int) -> Tuple[float, float]:
        lat_idx = pid // self.num_lon
        lon_idx = pid % self.num_lon
    ...
    def refresh_pid_members_and_subgraphs(self,
        ...
        # 依當下位置重建 PID 成員
        for pid in range(self.num_pid):
            self.pid_members[pid].clear()
        ...
        for s in sat_ids:
            ...
            p = self.pid_of(float(lat), float(lon))
            self.pid_members[p].add(s)
            sat_pid[s] = p

        # 建子圖與連通分量
        self.pid_subgraphs.clear()
        self.pid_sat_comp.clear()
        for pid, members in self.pid_members.items():
            if not members:
        ...
        _alog("[PID] neighbors stats", {
            "num_pids": len(self.pid_members),
    ...
    def select_pid_agents_by_geo_center(self,
                                        sat_nadir_latlon: Dict[int, Tuple[float,float]],
        ...
        for pid, members in self.pid_members.items():
            if not members:
                new_agents[pid] = None
                self._agent_hold_counter[pid] = 0
                continue
            lat_c, lon_c = self._pid_center(pid)
            ...
            for s in members:
                ...
                if d < best_d:
                    best_d, best_sat = d, s
            old = self.pid_agent_sat.get(pid)
            if old in members and old is not None:
                held = self._agent_hold_counter.get(pid, 0)
                ...
                if held < MIN_AGENT_HOLD_STEPS or best_d + dist_switch_threshold_km >= d_old:
                    new_agents[pid] = old
                    self._agent_hold_counter[pid] = held + 1
                    continue
            new_agents[pid] = best_sat
            self._agent_hold_counter[pid] = 1
        self.pid_agent_sat = new_agents

# -------------------------------
# GatewayCache：依「當下 ISL」重建跨 PID 候選
# -------------------------------
class GatewayCache:
...
class SSSPCache:
    def __init__(self):
        self.cache: Dict[Tuple[int,int], Dict[int,int]] = {}  # (pid, src) -> prev dict

    def clear_all(self):
        """清掉所有單源樹快取（每個 snapshot 建議呼叫一次）。"""
        self.cache.clear()
    
    def get_path(self, Gp: nx.Graph, pid: int, src: int, dst: int) -> List[int]:
        key = (pid, src)
        ...
        if dst not in paths:
            raise nx.NetworkXNoPath(f"no path {src}->{dst} in pid {pid}")
        return paths[dst]

    def invalidate_pid(self, pid: int):
        keys = [k for k in self.cache.keys() if k[0] == pid]
        for k in keys:
...
# 單對 GS 路由（階層式核心）
# -------------------------------
def route_one_pair(router: VirtualPIDRouter,
                   gcache: GatewayCache,
                   sssp_cache: SSSPCache,
                   sat_graph: nx.Graph,             # 全衛星圖（edge["weight"] 已設）
                   pid_graph: nx.Graph,             # PID 商圖（由 router.pid_neighbors 建）
                   pid_of_sat: Callable[[int], int],
                   pid_subgraphs: Dict[int, nx.Graph],
                   pid_sat_comp: Dict[int, Dict[int,int]],
                   sat_neighbor_to_if_map: Dict[Tuple[int,int], Tuple[int,int]],
                   fstate: Dict[Tuple[int,int], Tuple[int,int,int]],
                   src_uplink_sat: int,
    ...
    def intra(pid: int, a: int, b: int) -> List[int]:
        if a == b:
            return [a]
        Gp = pid_subgraphs[pid]
        try:
            return sssp_cache.get_path(Gp, pid, a, b)
        except nx.NetworkXNoPath:
            inA = (a in Gp)
            inB = (b in Gp)
            comp_a = pid_sat_comp[pid].get(a)
            comp_b = pid_sat_comp[pid].get(b)
            print(
                f"[NOPATH] PID={pid} subgraph|V|={Gp.number_of_nodes()} "
                f"no path {a}->{b} (inA={inA}, inB={inB}, comp_a={comp_a}, comp_b={comp_b})"
            )
            raise

    # 1) 同 PID：直接群內到 downlink（最高效率）
    if src_pid == dst_pid:
        path = intra(src_pid, src_uplink_sat, dst_downlink_sat)
        stitch_sat_path(path, dst_gs_node_id, sat_neighbor_to_if_map, fstate, stitch_skip_samples)
        return

    # 2) PID 商圖上找 src_pid → dst_pid 的路徑（相鄰 PID 早退 + 標準跨區）
    try:
        pid_path = nx.shortest_path(pid_graph, src_pid, dst_pid)
    except nx.NetworkXNoPath:
        # 3) 兜底（僅當允許）：全域最短
        if ALLOW_GLOBAL_FALLBACK:
            print(f"[FALLBACK] PID graph disconnected {src_pid}->{dst_pid}, using global shortest.")
            ...
            print(f"[NOPATH] PID-graph disconnected {src_pid}->{dst_pid}")
        return
    ...
    for next_pid in pid_path[1:]:
        # 若在迭代中已進入 dst_pid，立刻 Early-Exit
        if current_pid == dst_pid:
            final_path = intra(dst_pid, current_sat, dst_downlink_sat)
            stitch_sat_path(final_path, dst_gs_node_id, sat_neighbor_to_if_map, fstate, stitch_skip_samples)
            break

        # 候選 gateways
        candidates = gcache.get(current_pid, next_pid)

        # --- 同分量過濾 ---
        comp_cur = pid_sat_comp[current_pid].get(current_sat)
        comp_dst = pid_sat_comp[dst_pid].get(dst_downlink_sat) if next_pid == dst_pid else None

        def ok(trip):
            a, b, c = trip
            comp_a = pid_sat_comp[current_pid].get(a)
            if comp_cur is None or comp_a is None or comp_a != comp_cur:
                return False
            if comp_dst is not None:
                comp_b = pid_sat_comp[dst_pid].get(b)
        ...
        if not filtered:
            try:
                reachable = nx.single_source_shortest_path_length(pid_subgraphs[current_pid], current_sat)
            except Exception:
            ...
            for u in reachable.keys():
                for v in sat_graph.neighbors(u):
                    ...
                    if (u, v) not in sat_neighbor_to_if_map or (v, u) not in sat_neighbor_to_if_map:
                        continue
                    # 分量約束（嚴格）
                    comp_u = pid_sat_comp[current_pid].get(u)
                    if comp_cur is None or comp_u is None or comp_u != comp_cur:
                        continue
                    if next_pid == dst_pid:
                        comp_v = pid_sat_comp[dst_pid].get(v)
        ...
        # --- 還是沒有？改用「替代 next_pid」：在 current_pid 的其他鄰 PID 找可行 gateway ---
        if not filtered:
            alternatives = []
            for alt_pid in router.pid_neighbors[current_pid]:
                # 必須確保 alt_pid 在 PID 圖上仍能到 dst_pid
                try:
                    _ = nx.shortest_path(pid_graph, alt_pid, dst_pid)
                except nx.NetworkXNoPath:
                    continue
                # 從 (current_pid, alt_pid) 候選中找符合分量且真實 ISL 的
                for (a, b, c) in gcache.get(current_pid, alt_pid):
                    comp_a = pid_sat_comp[current_pid].get(a)
                    if comp_cur is None or comp_a is None or comp_a != comp_cur:
                        continue
                    if (a, b) not in sat_neighbor_to_if_map or (b, a) not in sat_neighbor_to_if_map:
            ...
            if alternatives:
                ...
                print(f"[ALT-NEXTPID] change {current_pid}->{next_pid} to {current_pid}->{alt_pid} due to no gateway from current component")
                next_pid = alt_pid
                filtered = [(a, b, c)]

        if not filtered:
            # 真正無解才報 NOPATH（現在比原本更早攔住不通情況）
            print(f"[NOPATH] no usable gateway {current_pid}->{next_pid} from {current_sat} (comp={comp_cur})")
            return

        gw_used = None
        for (a, b, c) in filtered:
            # 成員資格檢查
            if (a not in pid_subgraphs[current_pid]) or (b not in pid_subgraphs.get(next_pid, nx.Graph())):
                continue
            # 分量一致（安全再檢一次，避免任何不同步）
            comp_a = pid_sat_comp[current_pid].get(a)
            if comp_cur is None or comp_a is None or comp_a != comp_cur:
        ...
        if gw_used is None:
            print(f"[NOPATH] filtered gateways unusable after membership/comp checks {current_pid}->{next_pid} from {current_sat}")
            return
        ...
        if current_sat != satA:
            try:
                pathA = intra(current_pid, current_sat, satA)
        ...
        print(f"[GATEWAY] {t_label} PID {current_pid}->{next_pid} via {satA}->{satB}")
...
# -------------------------------
# 建 PID 商圖（from router.pid_neighbors）
# -------------------------------
def build_pid_graph(router: VirtualPIDRouter) -> nx.Graph:
    Gp = nx.Graph()
    Gp.add_nodes_from(range(router.num_pid))
    for a, neighs in router.pid_neighbors.items():
        for b in neighs:
            Gp.add_edge(a, b)  # 無權重；表示相鄰允許
    return Gp

def build_pid_constrained_sat_graph(
    G_sat_isls: nx.Graph,
    sat_pid: Dict[int, int],
    gateway_cache: Optional["GatewayCache"],
    router: VirtualPIDRouter
) -> nx.Graph:
    """
    基於分群規則建立「受限衛星圖」：
...
def route_all_gs_pairs(
    ...
    """
    # 1) 依當下位置重建 PID 成員與子圖/分量
    # 每個 snapshot 先重置邊權重為幾何長度
    reset_edge_weights_to_geo(G_sat_isls)
    sat_pid = router.refresh_pid_members_and_subgraphs(sat_ids, sat_nadir_latlon, G_sat_isls)
    
    # 清空群內 single-source shortest path 快取，避免用到上一個 snapshot
    sssp_cache.clear_all()

    # 2) （可選）選 agent（若其他模組要用）
    router.select_pid_agents_by_geo_center(sat_nadir_latlon)

    # 3) 重建 Gateway 候選（以當下實體 ISL）
    def edge_cost(u, v, data):
        return data.get("weight", data.get("geo_len_m", 1.0))
    gcache.rebuild(G_sat_isls, sat_pid, router.pid_neighbors, edge_cost)

    # 4) 建 PID 商圖（相鄰允許）
    pid_graph = build_pid_graph(router)
    ...
    for (_src_gs_node_id, dst_gs_node_id, src_uplink_sat, dst_downlink_sat, t_int) in gs_pairs:
        ...
        # 這些變數只在迴圈內才有
        src_pid = _pid_of_sat(src_uplink_sat)
        dst_pid = _pid_of_sat(dst_downlink_sat)
        t_label = f"t={t_int}"

        # 子圖更新後，清掉該兩個 PID 的群內快取，避免使用舊 prev
        sssp_cache.invalidate_pid(src_pid)
        sssp_cache.invalidate_pid(dst_pid)

        route_one_pair(router, gcache, sssp_cache,
                       G_sat_isls, pid_graph, _pid_of_sat,
                       router.pid_subgraphs, router.pid_sat_comp,
                       sat_neighbor_to_if_map, fstate,
                       src_uplink_sat, dst_downlink_sat,
                       src_pid, dst_pid, dst_gs_node_id,
                       t_label=t_label)
...
def step(payload: dict):
    ...
    若 MODE_SP_OVER_PID_QUOTIENT=True → 走 Hypatia 最短路（在 PID 受限圖上）。
    否則 → 走原本逐跳分層（stitch）流程。
    """
    assert _ROUTER and _GCACHE and _SSSP, "Call init() first."

    # 讓 log 一眼看出現在跑哪種模式
    print(f"[MODE] SP_OVER_PID_QUOTIENT={MODE_SP_OVER_PID_QUOTIENT}")

    _alog(f"[MODE] SP_OVER_PID_QUOTIENT={MODE_SP_OVER_PID_QUOTIENT}", payload)
    ...
    if MODE_SP_OVER_PID_QUOTIENT:
        ...
        # (4) Hypatia 最短路 - 使用優化的 Dijkstra 版本
        _alog("[SP] calling calculate_fstate_dijkstra_based on constrained graph", payload)
