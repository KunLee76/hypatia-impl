"""
Compare RTT performance between Baseline, GRHR, and LoHi algorithms.
...
def plot_comparison(baseline_data, grhr_data, lohi_data, src_id, dst_id, src_name, dst_name, constellation_name, duration_s, output_path):
    """
    Plot RTT comparison between Baseline, GRHR, and LoHi.
    
    Args:
        baseline_data: List of (time_ns, rtt_ns) for Baseline
    ...
    if baseline_data:
        baseline_rtts_valid = [r for r in baseline_rtts if r > 0]
        if baseline_rtts_valid:
            print(f"\nBaseline:")
            print(f"  - Data points: {len(baseline_data)}")
            print(f"  - Min RTT: {min(baseline_rtts_valid):.2f} ms")
            print(f"  - Max RTT: {max(baseline_rtts_valid):.2f} ms")
            print(f"  - Avg RTT: {sum(baseline_rtts_valid)/len(baseline_rtts_valid):.2f} ms")
            stats_dict['Baseline'] = sum(baseline_rtts_valid)/len(baseline_rtts_valid)
            unreachable_count = len([r for r in baseline_rtts if r == 0])
            if unreachable_count > 0:
                print(f"  - Unreachable: {unreachable_count} / {len(baseline_data)} ({unreachable_count/len(baseline_data)*100:.1f}%)")
    ...
    if grhr_data:
        ...
        if grhr_rtts_valid:
            ...
            stats_dict['GRHR'] = sum(grhr_rtts_valid)/len(grhr_rtts_valid)
    ...
    if lohi_data:
        ...
        if lohi_rtts_valid:
            ...
            stats_dict['LoHi'] = sum(lohi_rtts_valid)/len(lohi_rtts_valid)
    ...
    if len(stats_dict) >= 2:
        print(f"\nComparison:")
        if 'Baseline' in stats_dict and 'GRHR' in stats_dict:
            improvement = ((stats_dict['Baseline'] - stats_dict['GRHR']) / stats_dict['Baseline']) * 100
            if improvement > 0:
                print(f"  🎯 GRHR is {improvement:.1f}% better than Baseline")
            else:
                print(f"  🎯 Baseline is {-improvement:.1f}% better than GRHR")
        
        if 'Baseline' in stats_dict and 'LoHi' in stats_dict:
            improvement = ((stats_dict['Baseline'] - stats_dict['LoHi']) / stats_dict['Baseline']) * 100
            if improvement > 0:
                print(f"  🎯 LoHi is {improvement:.1f}% better than Baseline")
            else:
                print(f"  🎯 Baseline is {-improvement:.1f}% better than LoHi")
        
        if 'GRHR' in stats_dict and 'LoHi' in stats_dict:
            improvement = ((stats_dict['LoHi'] - stats_dict['GRHR']) / stats_dict['LoHi']) * 100
            if improvement > 0:
    ...
    if not baseline_candidates and not grhr_candidates and not lohi_candidates:
        print(f"\n❌ Error: Could not find any algorithm directories for {constellation_name}!")
        print(f"  - Baseline directory (should contain 'algorithm_free_one_only_over_isls_with_stats')")
        print(f"  - GRHR directory (should contain 'algorithm_hierarchical_virtual_gid')")
        print(f"  - LoHi directory (should contain 'algorithm_lohi')")
        sys.exit(1)
    
    baseline_pattern = baseline_candidates[0] if baseline_candidates else None
    grhr_pattern = grhr_candidates[0] if grhr_candidates else None
    lohi_pattern = lohi_candidates[0] if lohi_candidates else None
    ...
    base_pattern = None
    if lohi_pattern:
        base_pattern = lohi_pattern.replace('_algorithm_lohi', '')
    elif grhr_pattern:
        base_pattern = grhr_pattern.replace('_algorithm_hierarchical_virtual_gid', '').split('_27deg')[0]
    elif baseline_pattern:
        base_pattern = baseline_pattern.replace('_algorithm_free_one_only_over_isls_with_stats', '')
    
    gs_file = None
    
    # Search in satellite_networks_state/gen_data/
    potential_gs_paths = [
        os.path.join(data_dir, "..", "..", "satellite_networks_state", "gen_data", base_pattern, "ground_stations.txt"),
    ]
    
    if lohi_pattern:
        potential_gs_paths.append(os.path.join(data_dir, "..", "..", "satellite_networks_state", "gen_data", lohi_pattern, "ground_stations.txt"))
    if grhr_pattern:
    ...
    # Read data
    baseline_data = read_rtt_file(baseline_file) if baseline_file else []
    grhr_data = read_rtt_file(grhr_file) if grhr_file else []
    lohi_data = read_rtt_file(lohi_file) if lohi_file else []
    
    if not baseline_data and not grhr_data and not lohi_data:
        print("\n❌ Error: No data found in any file!")
        sys.exit(1)
    
    if not baseline_data:
        print("\n⚠️  Warning: No Baseline data found")
    if not grhr_data:
