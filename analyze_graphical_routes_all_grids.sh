...
# ============================================
# 生成不同網格大小的全球路由視角圖
# 執行 satgen.post_analysis.main_print_graphical_routes_and_rtt
# ============================================
...
echo "============================================"
echo ""
echo -e "${CYAN}⚠️  注意: 此腳本會生成大量PDF文件（每個時間點一張圖）${NC}"
echo -e "${CYAN}   建議先運行少量測試確認輸出正常${NC}"
echo ""
...
for ALGORITHM in "algorithm_hierarchical_virtual_pid" "algorithm_hierarchical_virtual_pid_dijkstra"; do
    ...
    for GRID_DEG in "${GRID_SIZES[@]}"; do
        ...
        for DST_GS in "${!DST_GS_MAP[@]}"; do
            ...
            echo -e "${CYAN}   生成全球視角圖（可能需要較長時間）...${NC}"
            ...
            echo -e "${GREEN}✅ 完成: ${ALGO_NAME} ${GRID_DEG}° Tokyo→${DST_NAME} (耗時: ${ELAPSED}s)${NC}"
            echo ""
        done
...
echo -e "${GREEN}============================================${NC}"
echo -e "${GREEN}🎉 所有全球視角圖生成完成！${NC}"
echo -e "${GREEN}============================================${NC}"
echo ""
echo "結果保存在:"
echo "  ${DATA_DIR}/starlink_550_isls_plus_grid_ground_stations_top_100_*/100ms_for_20s/manual/pdf/"
echo ""
echo "輸出文件命名格式:"
echo "  graphics_${SRC_GS}_to_[DST]_time_[時間]ms.pdf"
echo ""
echo "檢查輸出範例:"
echo "  ls -lh ${DATA_DIR}/starlink_550_isls_plus_grid_ground_stations_top_100_*_15deg/100ms_for_20s/manual/pdf/"
